...
#### Filesystem Hierarchy Analysis Script
```bash
#!/bin/bash
# fhs-analysis.sh - Comprehensive filesystem hierarchy analysis
...
analyze_directory() {
    ...
    if [[ -d "$dir" ]]; then
        ...
        echo "Directories: $(find "$dir" -type d 2>/dev/null | wc -l)"
        echo "Permissions: $(stat -c "%A %U:%G" "$dir" 2>/dev/null)"
        echo ""
    fi
}

echo "=== Linux Filesystem Hierarchy Analysis ==="
echo "Generated on: $(date)"
echo ""

# Core system directories
analyze_directory "/" "Root directory"
analyze_directory "/bin" "Essential user binaries"
analyze_directory "/sbin" "System binaries"
analyze_directory "/lib" "Essential shared libraries"
analyze_directory "/lib64" "64-bit shared libraries"

# Configuration and variable data
analyze_directory "/etc" "System configuration files"
analyze_directory "/var" "Variable data files"
analyze_directory "/var/log" "System log files"
analyze_directory "/var/spool" "Spool directories"

# User and application directories
analyze_directory "/home" "User home directories"
analyze_directory "/usr" "User programs and data"
analyze_directory "/usr/bin" "User binaries"
analyze_directory "/usr/sbin" "Non-essential system binaries"
analyze_directory "/usr/lib" "Libraries for /usr/bin and /usr/sbin"
analyze_directory "/opt" "Optional application software"

# Temporary and runtime
analyze_directory "/tmp" "Temporary files"
analyze_directory "/var/tmp" "Temporary files (preserved across reboots)"
analyze_directory "/run" "Runtime variable data"

# Virtual filesystems
analyze_directory "/proc" "Process and kernel information"
analyze_directory "/sys" "System and hardware information"
analyze_directory "/dev" "Device files"

# Mount points
analyze_directory "/mnt" "Temporary mount points"
analyze_directory "/media" "Removable media mount points"

echo "=== Analysis Complete ==="
```

#### Directory Purpose and Usage Guide
```bash
#!/bin/bash
# directory-guide.sh - Interactive filesystem hierarchy guide
...
show_directory_info() {
    ...
    case "$dir" in
        "/")
            echo "Root Directory (/)"
            echo "- The top-level directory of the entire filesystem"
            echo "- All other directories branch from here"
            echo "- Contains essential system directories"
            ;;
        "/bin")
            echo "Essential User Binaries (/bin)"
            echo "- Commands needed in single-user mode"
            echo "- Basic utilities like ls, cp, mv, cat, bash"
            echo "- Available to all users"
            ;;
        "/sbin")
            echo "System Binaries (/sbin)"
            echo "- System administration commands"
            echo "- Commands like fdisk, mount, fsck"
            echo "- Typically restricted to root user"
            ;;
        "/etc")
            echo "Configuration Files (/etc)"
            echo "- System-wide configuration files"
            echo "- No executable programs stored here"
            echo "- Examples: /etc/passwd, /etc/fstab, /etc/ssh/"
            ;;
        "/var")
            echo "Variable Data (/var)"
            echo "- Files that change during system operation"
            echo "- Log files, spool files, cache data"
            echo "- Subdirectories: /var/log, /var/spool, /var/cache"
            ;;
        "/usr")
            echo "User Programs (/usr)"
            echo "- Secondary hierarchy for user data"
            echo "- Most user programs and their data"
            echo "- Subdirectories: /usr/bin, /usr/lib, /usr/share"
            ;;
        "/home")
            echo "User Home Directories (/home)"
            echo "- Personal directories for users"
            echo "- User data and personal configuration"
            echo "- Each user has /home/username"
            ;;
        "/opt")
            echo "Optional Software (/opt)"
            echo "- Third-party application software"
            echo "- Self-contained application packages"
            echo "- Each application in /opt/application-name"
            ;;
        "/tmp")
            echo "Temporary Files (/tmp)"
            echo "- Temporary files created by applications"
            echo "- May be cleared on reboot"
            echo "- World-writable with sticky bit"
            ;;
        *)
            echo "Directory not in guide: $dir"
            ;;
    esac
    
    if [[ -d "$dir" ]]; then
        echo ""
        echo "Current contents (top 10):"
        ls -la "$dir" 2>/dev/null | head -11
    fi
}

# Interactive mode or direct lookup
if [[ $# -eq 0 ]]; then
    echo "=== Linux Filesystem Hierarchy Guide ==="
    echo "Enter directory path (or 'quit' to exit):"
    ...
    while true; do
        ...
        echo ""
        show_directory_info "$dir"
        echo ""
    done
else
    show_directory_info "$1"
fi
```
